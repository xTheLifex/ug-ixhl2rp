...
PLUGIN.name = "Better Areas"
PLUGIN.author = "TheLife & `impulse"
PLUGIN.authorColor = Color(55,180,230)
PLUGIN.description = "Provides customizable area definitions, not limited to one piece per area."

if (ix.plugin.Get("area")) then
	ix.plugin.list["area"] = nil
	MsgC(Color(255,0,0), string.format("[%s] Trying to load the plugin while the default helix areas are still loaded. This can cause problems!", PLUGIN.name))
end
